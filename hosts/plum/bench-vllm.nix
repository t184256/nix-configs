...
let
  bench-vllm = pkgs.writeShellScriptBin "bench-vllm" ''
    set -euo pipefail

    PORT=8001
    URL="http://127.0.0.1:$PORT"
    MODEL="qwen35-9b"  # vllm uses the last component of the model dir path

    PROMPT_LENS_PLAIN="256,8192,16384,32768,65536,131072,196608,256000"
    PROMPT_LENS_DFLASH="256,8192,16384,32768,65536"  # dflash max_model_len ~92k
    GEN_TOKENS=128
    RUNS=3

    usage() {
      echo "usage: bench-vllm [plain|dflash|both]" >&2
      exit 1
    }
    ...
    def bench_once(approx_tokens):
        # Unique prefix busts the vllm prefix cache between runs.
        _run_counter[0] += 1
        ...
        prompt_tokens = data.get("usage", {}).get("prompt_tokens", approx_tokens)
        return prompt_tokens, ttft, decode_tps, n_tokens

    print(f"  {'target':>8}  {'pp':>6}  {'ttft':>8}  {'pp t/s':>8}  {'decode t/s':>12}  {'gen':>5}")
    ...
    for approx in prompt_lens:
        ...
        for _ in range(int(runs)):
            ...
            actual_pp, actual_gen = pp, gen
            ttfts.append(ttft)
            tpss.append(tps)
        med_ttft = statistics.median(ttfts)
        med_tps  = statistics.median(tpss)
        pp_tps = actual_pp / med_ttft if med_ttft > 0 else 0
        print(f"  {approx:>8}  {actual_pp:>6}  {med_ttft:>7.3f}s  {pp_tps:>7.0f}  {med_tps:>11.1f}  {actual_gen:>5}")
    PYEOF
    ...
    case "$MODE" in
      plain)  PROMPT_LENS="$PROMPT_LENS_PLAIN"  run_bench vllm        "9B AWQ plain" ;;
      dflash) PROMPT_LENS="$PROMPT_LENS_DFLASH" run_bench vllm-dflash "9B AWQ + DFlash" ;;
      both)
        PROMPT_LENS="$PROMPT_LENS_PLAIN"  run_bench vllm        "9B AWQ plain"
        PROMPT_LENS="$PROMPT_LENS_DFLASH" run_bench vllm-dflash "9B AWQ + DFlash"
        ;;
    esac
  '';
...
{
  environment.systemPackages = [ bench-vllm ];
}
