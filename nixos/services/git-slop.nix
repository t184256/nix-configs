...

{
  services.forgejo = {
    ...
    settings = {
      ...
      server = {
        DOMAIN = "git.slop.unboiled.info";
        HTTP_ADDRESS = "127.0.0.1";
        HTTP_PORT = 3000;
        ROOT_URL = "https://git.slop.unboiled.info";
      };
      service = {
      ...
      DEFAULT.APP_NAME = "monk's bot's private forgejo";
    };
  };
  ...
  services.nginx = {
    enable = true;
    virtualHosts."git.slop.unboiled.info" = {
      enableACME = true;
      forceSSL = true;
  ...
  services.restic.backups.forgejo-slop = {
    environmentFile = "/mnt/storage/secrets/restic/forgejo-slop/key";
    passwordFile = "/mnt/storage/secrets/restic/forgejo-slop/pass";
    paths = [ "service-forgejo-slop" ];
    repository =
      "s3:http://localhost:3900/service-forgejo-slop-"
      + config.networking.hostName;
    pruneOpts = [ "--keep-daily 30" "--keep-weekly 30" "--keep-monthly 30" ];
    backupPrepareCommand = ''
      ${pkgs.btrfs-progs}/bin/btrfs subvolume snapshot -r \
        ${config.services.forgejo.stateDir} \
        /mnt/storage/tmp/service-forgejo-slop
    '';
  ...
  systemd.services.restic-backups-forgejo-slop = {
