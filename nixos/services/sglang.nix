...
let
  ...
  sglangPkg = (import "${flox}/sglang-pkg.nix" {
    ...
    patches = (old.patches or []) ++ [
      # PyNcclCommunicator.device missing on disabled comms; see:
      # sglang/srt/distributed/device_communicators/pynccl.py early returns.
      ../../overlays/sglang/pynccl-device-before-early-return.patch
    ];
  });
  ...
  commonEnv = [
    ...
    # Two RTX 3090s without NVLink; PCIe P2P not supported on this board.
    # NCCL falls back to host-memory staging for inter-GPU transfers.
    # Remove if P2P becomes available (NVLink or board that supports PCIe P2P).
    "NCCL_P2P_DISABLE=1"
  ];
...

{
  ...
  systemd.services.sglang = lib.recursiveUpdate commonService {
    ...
    unitConfig.Conflicts = "sglang-dflash.service vllm.service vllm-dflash.service";
  ...
  systemd.services.sglang-dflash = lib.recursiveUpdate commonService {
    ...
    unitConfig.Conflicts = "sglang.service vllm.service vllm-dflash.service";
